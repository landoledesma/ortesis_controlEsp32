...
#from sh1106 import SH1106_I2C # Library for Oled
...
i2c_IMU = SoftI2C(scl=machine.Pin(22), sda=machine.Pin(21), freq=400000) # IMU 

def init_MPU():
    #write to sample rate register 
    i2c_IMU.writeto_mem(mpu6050_addr, SMPLRT_DIV, b'\x07')
    #Write to power management register to wake up mpu6050
    i2c_IMU.writeto_mem(mpu6050_addr, PWR_MGMT_1, b'\x00')
    #Write to Configuration register 
    i2c_IMU.writeto_mem(mpu6050_addr, CONFIG, b'\x00')
    #Write to Gyro configuration register to self test gyro 
    i2c_IMU.writeto_mem(mpu6050_addr, GYRO_CONFIG, b'\x18')
    #Set interrupt enable register to 0 .. disable interrupts
    i2c_IMU.writeto_mem(mpu6050_addr, INT_ENABLE, b'\x00')

def read_raw_data(addr):
    #Accelero and Gyro value are 16-bit
    high = i2c_IMU.readfrom_mem(mpu6050_addr, addr, 1)
    #print(ubinascii.hexlify(high))
    low  = i2c_IMU.readfrom_mem(mpu6050_addr, addr+1, 1)
